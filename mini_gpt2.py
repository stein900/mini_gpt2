...
class AddLineDataset(torch.utils.data.Dataset):
    ...
    Échantillon = "A + B = C\n"
    input = ids[:-1]
    labels = ids[1:], avec labels[:P-1] = -100   (P = len(prompt))
    → on entraîne dès la 1ʳᵉ cible de la réponse (le 1er chiffre)
    """
    def __init__(self, path, tok: ByteTokenizer, max_len_hint=32):
        ...
        for ln in lines:
            parts = ln.split(' ')
            assert len(parts) == 5 and parts[1] == '+' and parts[3] == '=', f"Ligne invalide: {ln!r}"
            a, b, c = parts[0], parts[2], parts[4]
            prompt = f"{a} + {b} = "
            answer = f"{c}\n"
    ...
    def __getitem__(self, idx):
        ...
        # ---- CORRECTION ICI ----
        # masque seulement P-1 premières positions (P = len(prompt))
        P = len(p_ids)
        mask_len = max(0, P - 1)
        for i in range(mask_len):
            lab[i] = -100

        # padding à longueur fixe (utilise 'espace' comme pad pour éviter un biais vers '0')
        PAD = 32  # ord(' ')
